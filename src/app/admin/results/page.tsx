...
import { Search, Eye, Pencil, Trash2, FileDown, FileSpreadsheet, Loader2, FileText, Filter } from "lucide-react";
...
export default function ResultsPage() {
  ...
  const updateMutation = useApiMutation<StudentWithId, Partial<Student>>(selectedStudent ? apiRoutes.studentById(selectedStudent._id) : "", {
    method: "PUT",
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: queryKeys.students() }); setEditOpen(false); setSelectedStudent(null); showSuccess("طالب علم اپ ڈیٹ ہو گیا۔"); },
    onError: (err) => showError(err.message),
  });

  const deleteMutation = useApiMutation<{ success: boolean }>(selectedStudent ? apiRoutes.studentById(selectedStudent._id) : "", {
    method: "DELETE",
    onSuccess: () => { void queryClient.invalidateQueries({ queryKey: queryKeys.students() }); setDeleteOpen(false); setSelectedStudent(null); showSuccess("طالب علم حذف ہو گیا۔"); },
  ...
  const filteredStudents = useMemo(() => {
    ...
    return list.filter((s) => {
      ...
      const matchesSearch = term.length === 0 || s.name.toLowerCase().includes(term) || s.rollNumber.toLowerCase().includes(term) || s.registrationNumber.toLowerCase().includes(term) || s.email.toLowerCase().includes(term);
  ...
  const handleExportExcel = useCallback(async () => {
    ...
    try {
      ...
      link.download = filterYear ? `students-results-${filterYear}.xlsx` : "students-results-all.xlsx";
  ...
  const columns: Column<StudentWithId>[] = [
    { key: "name", header: "طالب علم", render: (row) => (<div><p className="font-semibold text-gray-900">{row.name}</p><p className="text-xs text-gray-400 mt-0.5">{row.email}</p></div>) },
    ...
    { key: "regNumber", header: "رجسٹریشن", render: (row) => <span className="font-mono text-xs text-gray-500 bg-gray-50 px-2 py-1 rounded-md">{row.registrationNumber}</span> },
    { key: "class", header: "جماعت", render: (row) => <span className="px-2.5 py-1 bg-violet-50 text-violet-700 rounded-lg text-xs font-semibold border border-violet-100">{getClassName(row.classId)}</span> },
    {
      key: "actions", header: "عمل", render: (row) => (
        <div className="flex items-center gap-1.5">
          <button onClick={() => { setSelectedStudent(row); setViewOpen(true); }} className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-indigo-700 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition border border-indigo-100"><Eye className="w-3.5 h-3.5" /> دیکھیں</button>
  ...
  const editInitialData: (Partial<Student> & { _id?: string }) | null = selectedStudent ? { _id: selectedStudent._id, registrationNumber: selectedStudent.registrationNumber, rollNumber: selectedStudent.rollNumber, name: selectedStudent.name, parentName: selectedStudent.parentName, email: selectedStudent.email, phone: selectedStudent.phone, CNIC: selectedStudent.CNIC, classId: getClassId(selectedStudent.classId) } : null;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5 hidden sm:block">طلباء کے ریکارڈ دیکھیں، ترمیم کریں اور پی ڈی ایف ڈاؤن لوڈ کریں</p>
          </div>
        </div>
        <button onClick={handleExportExcel} disabled={exporting} className="px-3 sm:px-4 py-2 sm:py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-medium hover:bg-emerald-700 transition flex items-center gap-2 disabled:opacity-50 shadow-sm shadow-emerald-600/20 self-start sm:self-auto shrink-0">
          {exporting ? <><Loader2 className="w-4 h-4 animate-spin" /> ...</> : <><FileSpreadsheet className="w-4 h-4" /> ایکسل</>}
        </button>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        <div className="relative flex-1 min-w-0">
          ...
          <input type="text" placeholder="نام، رول نمبر، رجسٹریشن یا ای میل سے تلاش..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pe-10 ps-4 py-2.5 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 hover:border-gray-300 transition-all" />
        </div>
        <div className="flex items-center gap-2">
          <Filter className="w-4 h-4 text-gray-400" />
          <select value={filterYear} onChange={(e) => { setFilterYear(e.target.value); setFilterClassId(""); }} className={`${SELECT_CLASS} min-w-[130px]`}>
            <option value="">تمام سال</option>
            {availableYears.map((y) => (<option key={y} value={String(y)}>{y}</option>))}
          </select>
          <select value={filterClassId} onChange={(e) => setFilterClassId(e.target.value)} className={`${SELECT_CLASS} min-w-[160px]`}>
      ...
      <DataTable columns={columns} data={filteredStudents} rowKey={(row) => row._id} loading={isLoading} emptyMessage="آپ کی تلاش سے کوئی طالب علم نہیں ملا۔" />
      <ResultCardModal open={viewOpen} onClose={() => { setViewOpen(false); setSelectedStudent(null); }} student={selectedStudent} />
      ...
      <ConfirmDialog open={deleteOpen} onClose={() => { setDeleteOpen(false); setSelectedStudent(null); }} onConfirm={handleDelete} title="طالب علم حذف" message={`کیا آپ واقعی "${selectedStudent?.name ?? ""}" حذف کرنا چاہتے ہیں؟`} loading={deleteMutation.isPending} />
    </div>
  );
}
