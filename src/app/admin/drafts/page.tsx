...
export default function DraftsPage() {
  ...
  const handleAssign = useCallback(
    async (student: StudentWithId) => {
      const classId = assignMap[student._id];
      if (!classId) {
        showError("پہلے جماعت منتخب کریں");
        return;
      }
      setAssigningId(student._id);
      try {
        await apiRequest<StudentWithId>(apiRoutes.studentById(student._id), {
          method: "PUT",
          json: { classId },
        });
  ...
  const columns: Column<StudentWithId>[] = [
    ...
    {
      key: "assign",
      header: "جماعت تفویض کریں",
      render: (row) => (
        <div className="flex items-center gap-2">
          <select
            value={assignMap[row._id] ?? ""}
            onChange={(e) => handleClassSelect(row._id, e.target.value)}
            className="px-3 py-2 border border-gray-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 hover:border-gray-300 transition-all min-w-[140px]"
          >
            <option value="">جماعت منتخب کریں</option>
            {(classes ?? []).map((c: ClassWithId) => (
          ...
          <button
            ...
            disabled={!assignMap[row._id] || assigningId === row._id}
            className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-sm shadow-blue-600/20"
          >
            {assigningId === row._id ? (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5 hidden sm:block">
            وہ طلباء جن کی جماعت حذف ہو چکی ہے — انہیں نئی جماعت تفویض کریں
          </p>
        </div>
      </div>
      ...
      {!isLoading && drafts && drafts.length > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-amber-100 bg-amber-50/80 px-4 py-3.5">
          ...
          <p className="text-sm text-amber-800 font-medium">
            <strong>{drafts.length}</strong> طالب علم کو جماعت تفویض کرنے کی ضرورت ہے
          </p>
        </div>
      )}
      ...
      <DataTable
        ...
        rowKey={(row) => row._id}
        loading={isLoading}
        emptyMessage="کوئی غیر مختص طالب علم نہیں — تمام طلباء جماعت میں ہیں۔"
      />
    </div>
  );
}
