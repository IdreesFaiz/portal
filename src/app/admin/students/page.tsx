  ...
  const updateMutation = useApiMutation<StudentWithId, Partial<Student>>(
    selectedStudent ? apiRoutes.studentById(selectedStudent._id) : "",
    {
      ...
      onSuccess: () => {
        ...
        showSuccess("طالب علم کامیابی سے اپ ڈیٹ ہو گیا۔");
      },
  ...
  const deleteMutation = useApiMutation<{ success: boolean }>(
    selectedStudent ? apiRoutes.studentById(selectedStudent._id) : "",
    {
      ...
      onSuccess: () => {
        ...
        setDeleteOpen(false);
        setSelectedStudent(null);
        showSuccess("طالب علم کامیابی سے حذف ہو گیا۔");
      },
      onError: (err) => {
  ...
  const handleAdd = useCallback(
    (data: Student) => {
      createMutation.mutate(data, {
        onSuccess: () => {
          setAddOpen(false);
          showSuccess("طالب علم کامیابی سے شامل ہو گیا۔");
        },
        onError: (err) => showError(err.message),
      });
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5 self-start sm:self-auto">
          <button
            ...
            disabled={exporting}
            className="px-3 sm:px-4 py-2 sm:py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-medium hover:bg-emerald-700 transition flex items-center gap-2 disabled:opacity-50 shadow-sm shadow-emerald-600/20"
          >
            {exporting ? (
              <>
              ...
              <>
                ...
                <span className="hidden sm:inline"> ایکسل</span>
              </>
            )}
          </button>
          <button
            onClick={() => setAddOpen(true)}
            className="px-3 sm:px-4 py-2 sm:py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition flex items-center gap-2 shadow-sm shadow-blue-600/20"
          >
            <Plus className="w-4 h-4" /> <span className="hidden sm:inline">نیا طالب علم</span>
            <span className="sm:hidden">نیا</span>
          </button>
        </div>
      </div>
      ...
      <DataTable
        ...
        rowKey={(row) => row._id}
        loading={isLoading}
        emptyMessage="کوئی طالب علم نہیں ملا۔ 'نیا طالب علم' پر کلک کریں۔"
      />
      <StudentFormModal
        open={addOpen}
      ...
      <ConfirmDialog
        ...
        message={`کیا آپ واقعی "${selectedStudent?.name ?? ""}" کو حذف کرنا چاہتے ہیں؟ یہ عمل واپس نہیں ہو سکتا۔`}
