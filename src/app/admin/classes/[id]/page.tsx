...
export default function ClassDetailPage({ params }: PageProps) {
  ...
  const { errorMsg, successMsg, showError, showSuccess, clearError, clearSuccess } =
    useNotification();
  const { data: classData, isLoading: classLoading } = useApiQuery<ClassWithId>(
    queryKeys.classById(id),
    apiRoutes.classById(id)
  );
  ...
  const promoteTargetOptions = useMemo(() => {
    ...
    return allClasses.filter((c) => c._id !== id);
  }, [allClasses, id]);
  ...
  const handlePromote = useCallback(async () => {
    ...
    setPromoting(true);
    try {
      const res = await apiRequest<{ promotedCount: number }>(apiRoutes.classPromote(id), {
        method: "POST",
        json: { targetClassId: promoteTargetId },
      });
  ...
  const studentColumns: Column<StudentWithId>[] = [
    ...
    },
    {
      key: "email",
      header: "ای میل",
      render: (row) => <span className="text-gray-600 text-xs">{row.email}</span>,
    },
    {
      key: "phone",
      header: "فون",
      render: (row) => <span className="text-gray-600 text-xs">{row.phone}</span>,
    },
    {
      key: "actions",
      header: "عمل",
  ...
  return (
    <div className="space-y-6">
      <Link
        href="/admin/classes"
        className="text-blue-600 hover:text-blue-800 text-sm inline-flex items-center gap-1 font-medium"
      >
        <ChevronRight className="w-4 h-4" /> تمام جماعتوں پر واپس
      </Link>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className="text-xs sm:text-sm text-gray-500">
                  {classData.year} &middot; {classData.courses.length} مضامین &middot; {totalMarks}{" "}
                  کل نمبر
                </span>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium text-purple-700 bg-purple-50 rounded-xl hover:bg-purple-100 transition border border-purple-100 shadow-sm self-start sm:self-auto shrink-0"
          >
          ...
          <div className="mt-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {classData.courses.map((course, idx) => (
                <div
                  key={idx}
                  className="bg-violet-50/60 border border-violet-100 rounded-xl p-3.5"
                >
      ...
      <div>
        <h2 className="text-lg font-bold text-gray-900 mb-3">
          اس جماعت کے طلباء ({students?.length ?? 0})
        </h2>
        <DataTable
          ...
          emptyMessage="اس جماعت میں ابھی کوئی طالب علم نہیں۔"
        />
      </div>
        ...
        <div className="space-y-5">
          <div className="bg-purple-50/80 border border-purple-100 rounded-xl p-4">
            <p className="text-sm text-purple-800">
              <strong>تمام {students?.length ?? 0} طلباء</strong> کو{" "}
              <strong>
                {classData.className} ({classData.year})
              </strong>{" "}
              سے نئی جماعت میں منتقل کریں۔ پرانے نمبرات محفوظ رہیں گے۔
            </p>
          </div>
          <div>
