...
const emptyForm: Student = { registrationNumber: "", rollNumber: "", name: "", parentName: "", email: "", phone: "", CNIC: "", classId: "" };
...
export default function StudentFormModal(props: StudentFormModalProps) {
  ...
  return (
    <Modal open={props.open} onClose={handleClose} title={props.title ?? "نیا طالب علم"} maxWidth="max-w-2xl">
      {props.open && (
        <StudentFormInner key={mountKey} onClose={handleClose} onSubmit={props.onSubmit} loading={props.loading ?? false} initialData={props.initialData ?? null} />
      )}
    </Modal>
  );
}

function StudentFormInner({ onClose, onSubmit, loading, initialData }: {
  onClose: () => void; onSubmit: (data: Student) => void; loading: boolean; initialData: Partial<Student> | null;
}) {
  const [form, setForm] = useState<Student>(initialData ? { ...emptyForm, ...initialData } : emptyForm);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (UNIQUE_FIELDS.includes(name as UniqueField)) {
      ...
      setFieldErrors((prev) => { const next = { ...prev }; if (err) { next[name as keyof Student] = err; } else { delete next[name as keyof Student]; } return next; });
    }
  };

  const hasErrors = Object.keys(fieldErrors).length > 0;
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); if (hasErrors) return; onSubmit(form); };
  const selectedClass = classes?.find((c: ClassWithId) => c._id === form.classId);

  function renderField(name: keyof Student, label: string, type = "text", extra?: Record<string, string>) {
    const error = fieldErrors[name];
    return (
      <div>
        <label className="block mb-1.5 text-sm font-semibold text-gray-700">{label}</label>
        <input name={name} type={type} value={form[name] ?? ""} onChange={handleChange} required className={error ? INPUT_ERROR_CLASS : INPUT_CLASS} {...extra} />
        {error && (
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="classId" value={form.classId ?? ""} onChange={handleChange} required className={INPUT_CLASS}>
            <option value="">{classesLoading ? "لوڈ ہو رہا ہے..." : "جماعت منتخب کریں"}</option>
            {classes?.map((cls: ClassWithId) => (<option key={cls._id} value={cls._id}>{cls.className}</option>))}
          </select>
        </div>
      </div>
        ...
        <div className="bg-blue-50/60 rounded-xl p-4 border border-blue-100">
          ...
          <div className="flex flex-wrap gap-2">
            {selectedClass.courses.map((course, idx) => (
              <span key={idx} className="px-3 py-1.5 bg-white text-blue-700 rounded-lg text-xs font-medium border border-blue-100 shadow-sm">{course.name} ({course.marks} نمبر)</span>
            ))}
          </div>
        </div>
      )}
      ...
      {hasErrors && (
        <div className="bg-red-50/80 border border-red-100 rounded-xl px-4 py-3 flex items-center gap-2">
          ...
          <p className="text-sm text-red-700 font-medium">محفوظ کرنے سے پہلے اوپر والی خرابیاں ٹھیک کریں۔</p>
        </div>
      )}

      <div className="flex justify-start gap-3 pt-2">
        <button type="submit" disabled={loading || hasErrors} className="px-6 py-2.5 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition-all disabled:opacity-50 shadow-sm shadow-blue-600/20 flex items-center gap-2">
