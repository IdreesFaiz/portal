...
function ClassFormInner({
  ...
  const handleCourseChange = (index: number, field: keyof Course, value: string) => {
    setCourses((prev) => {
      const updated = [...prev];
      if (field === "marks") {
        updated[index] = { ...updated[index], marks: Math.max(0, Number(value) || 0) };
      } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      className,
      year,
      courses: courses.filter((c) => c.name.trim() !== "" && c.marks > 0),
      resultsPublished: initialData?.resultsPublished ?? false,
    });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            type="button"
            onClick={() => setCourses((prev) => [...prev, { name: "", marks: 0 }])}
            className="px-3.5 py-1.5 bg-emerald-600 text-white text-xs font-semibold rounded-lg hover:bg-emerald-700 transition flex items-center gap-1.5 shadow-sm shadow-emerald-600/20"
          >
              ...
              <div key={index} className="flex items-start gap-2.5">
                <div className="flex-1">
                  <input
                    ...
                    className={
                      isDupe
                        ? `${INPUT_BASE} border-red-300 focus:ring-red-400/30 focus:border-red-400 bg-red-50/50`
                        : INPUT_CLASS
                    }
                  />
                  {isDupe && (
                    <p className="mt-1 text-xs text-red-600 flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> مضمون کا نام دہرایا گیا ہے
                    </p>
                  )}
                </div>
                <input
                ...
                {courses.length > 1 && (
                  <button
                    ...
                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-600 transition mt-0.5"
                  >
      ...
      {hasDuplicates && (
        <div className="bg-red-50/80 border border-red-100 rounded-xl px-4 py-3 flex items-center gap-2">
          ...
          <p className="text-sm text-red-700 font-medium">
            محفوظ کرنے سے پہلے دہرائے گئے مضامین ٹھیک کریں۔
          </p>
        </div>
      )}
