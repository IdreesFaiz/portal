          ...
          <div className="bg-linear-to-r from-slate-800 to-slate-900 rounded-xl p-6 text-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-slate-300 text-sm mt-1">{className}</p>
              </div>
              {hasMarks && (
                <div
                  className={`px-4 py-2 rounded-lg text-center ${passed ? "bg-green-500/20 border border-green-400/30" : "bg-red-500/20 border border-red-400/30"}`}
                >
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              {courses.length > 0 ? (
                ...
                <p className="text-sm text-gray-400">کوئی مضمون نہیں</p>
              )}
            </div>
          </div>
          ...
          {hasMarks ? (
                      ...
                      return (
                        <tr key={cm.courseName} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-2.5 text-center">
                            <span
                              className={`text-xs font-medium ${subPassed ? "text-green-700" : "text-red-600"}`}
                            >
                              {pct.toFixed(1)}%
                            </span>
                          </td>
                          <td className="px-4 py-2.5 text-center">
                            <span
                              className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${subPassed ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                            >
                              {subPassed ? "پاس" : "فیل"}
                            </span>
                          </td>
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-6 text-center">
              ...
              <p className="text-yellow-600 text-xs mt-1">
                نمبرات کے اندراج سے اس طالب علم کے نمبرات شامل کریں۔
              </p>
            </div>
          )}
